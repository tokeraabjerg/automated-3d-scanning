...
def Preproces_normal_pipeline(pcd, voxel_size=0.1, std_ratio=2.0):
    ...
    logger.info(f"Voxel downsampling completed, points count: {len(pcd_voxel.points)}")

    pcd_normal = normal_space_sampling_with_bin_control(pcd_voxel, int(len(pcd_voxel.points)/12), radius=3, max_nn=50, bin_size=360)
    # Remove outliers
    logger.info("Starting outlier removal")
    pcd_downsampled, ind = pcd_normal.remove_statistical_outlier(nb_neighbors=20, std_ratio=std_ratio)
